...
const ChatRoom: React.FC<ChatRoomProps> = ({ otherUser, onBack, isDesktop }) => {
  ...
  const handleSend = (text?: string, type: 'text' | 'proposal' = 'text', proposalData?: any) => {
    ...
    if (type === 'text') {
      setTimeout(() => {
        setMessages(prev => [...prev, {
          ...
          senderId: otherUser.id,
          text: "That sounds like a great trade! When are you usually free for a call?",
          timestamp: new Date()
        }]);
      }, 1500);
    }
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.senderId === 'me' ? 'justify-end' : 'justify-start'}`}>
            {msg.type === 'proposal' ? (
              ...
              <div className={`max-w-[70%] px-5 py-3.5 rounded-2xl text-sm leading-relaxed ${
                msg.senderId === 'me' 
